...
class TestReviewCli:
    def test_review_no_id_and_no_wrong_flag_exits_nonzero(self, db_file: Path) -> None:
        result = runner.invoke(app, ["review", "--db", str(db_file)])
        assert result.exit_code != 0

    def test_review_nonexistent_db_exits_nonzero(self, tmp_path: Path) -> None:
        result = runner.invoke(app, ["review", "any_id", "--db", str(tmp_path / "missing.db")])
        assert result.exit_code != 0

    def test_review_unknown_feladat_id_exits_nonzero(self, db_file: Path) -> None:
        result = runner.invoke(app, ["review", "no_such_id", "--db", str(db_file)])
        assert result.exit_code != 0
        assert "nem található" in result.output

    def test_review_wrong_no_attempts_prints_message(self, db_file: Path) -> None:
    ...
    def test_review_versioned_reports_new_id(self, db_file: Path, feladat: Feladat) -> None:
    ...
    def test_review_diff_shown_for_changed_field(self, db_file: Path, feladat: Feladat) -> None:
        changed = dataclasses.replace(
            feladat,
            kerdes="Javított kérdés?",
            review_elvegezve=True,
        )
